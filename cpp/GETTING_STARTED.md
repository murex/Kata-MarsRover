# Getting Started with Mars Rover kata in C++
...
1. When executed on Windows and macOS, our script will, by default, generate the solution files ***Kata-MarsRover.sln*** for **'Visual Studio 2017'** & ***Kata-MarsRover.xcodeproj*** for **'Xcode'** respectively. 
...
```shell
    Start 1: ctest-Kata-MarsRover
1/1 Test #1: ctest-Kata-MarsRover ...........   Passed    1.21 sec
...
1. For *Windows*: The **'Visual Studio 2017'** solution file `build` / `Kata-MarsRover.sln`
1. For *macOS*: The **'Xcode'** project file `build` / `Kata-MarsRover.xcodeproj`

### Using IDEs 

The CMake file we provided generates the target 'Kata-MarsRover-test' that defines the test location (i.e. [MarsRoverTest.cpp](./test/MarsRoverTest.cpp)). 

#### Visual Studio 2017

You can open the project in VS 2017, by simply running the solution file `Kata-MarsRover` / `cpp` / `build` / `Kata-MarsRover.sln`

After loading the project into your IDE:

1. Make sure the 'Kata-MarsRover-test' project is set as the StartUp Project then run the project.
2. Run the project (in VS use '**F5**' or click on '**Local Window Debugger**' in the toolbar). The following output should be generated: 
    ```shell
    Running main() from .../Kata-MarsRover/cpp/build/_deps/googletest-src/googletest/src/gtest_main.cc
...
1. Start CLion and open the project from `Kata-MarsRover` / `cpp` folder. 
1. CLion should automatically build the CMake file. 
1. Run the test file [MarsRoverTest.cpp](./test/MarsRoverTest.cpp).
