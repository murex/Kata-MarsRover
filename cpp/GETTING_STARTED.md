...
- Generate the solution file ***Kata-MarsRover.sln*** for **Visual Studio 2022** on Windows,
...
Open Xcode, choose `Open a project or file`, navigate to `Kata-MarsRover` / `cpp` / `build` / `Kata-MarsRover.xcodeproj`
then click `Open`.

Once the project is opened in Xcode, select `Product` / `Scheme` / `Kata-MarsRover-test` to set it as the current active scheme.
