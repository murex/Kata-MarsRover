...
*⚠ This document contains information related to the solution for this katas.
...
- [Prepared Kata](../SessionFormats/PreparedKata.md)
- [Randori Kata](../SessionFormats/RandoriKata.md)
- [Randori in Pairs](../SessionFormats/RandoriInPairs.md)
- [Mob Programming](../SessionFormats/MobProgramming.md)

### Quick Retrospectives

Following each session, we run a quick retrospective that is 10 minutes long. The aim is to gather information on how the session went:

1. What We Did
1. What We Learned
1. What Puzzled Us
1. What We Decided to Do Next
...
1. Get feedback on the sessions and improve them for the future
1. Helps us un-cover some needs for the teams
...
1. Make sure to free the last 10 minutes for the retro
...
Any further kata teaches techniques, but the initial TDD part teaches a mindset and principles. It is the most important. It's easier to sell the coaching with the techniques and their benefits though. That's why we usually highlight refactoring improvements for example.

That being said, as coaches, we must not forget that the first TDD section is the most important one. If a team wants to go through the refactoring katas without first going through the TDD part, try to explain that most gains come from the first part. You might use previous coaching sessions
as social proof. If they still don't want to go through the TDD part, tell them to do it by themselves. The katas are made to be self-service, and a coach presence will be useless in this
case.
...
It's ok to try to understand for 1 minute or 2, but not longer. If you get in the situation, the best thing to do is to revert to the latest green state. Once all the tests are passing, redo the changes in extra-small-baby-steps, running the tests every time, until some test turns red. The reason of the failure should be obvious then.
...
When teams or pairs finish a kata in advance, a possible advice for "keeping them busy"
is to tell them to spend their remaining time reviewing and refactoring their tests:
...
1. Remind the full team (not a specific team) to switch pairs
...
- It's like Kent Beck's 3X framework (explore, expand, extract), or Basecamp's Hill progress ([Hill Charts](https://basecamp.com/hill-charts))
...
- When scheduling kata sessions, also schedule a reminder on the day before each session to create the branch
  for the kata and send them the name of the branch and instructions to checkout this branch.
  You can also point them to the section `When starting a new kata` in the [Getting Started](../GETTING_STARTED.md) document.
- Note that in case of Randori In Pairs sessions this will be harder to anticipate, as branch names
  will depend on how the pairs are organized. In this situation just point them to [Getting Started](../GETTING_STARTED.md)
  and be ready to help, using remote screen sharing if needed.
- If the problem persists, invite the team to an "Environment Clinic" session, where everyone in the team who
  is struggling to have their environment up and running can come and ask for help and advice.
...
For example, writing Cucumber scenarios. Adding those tools and libraries might not be a smooth task due to the various dependencies in the code. Doing this configuration with the full team might waste the full session and make the team frustrated.
...
1. Ensure they have a working environment ready for the coming mob/kata (ex: ensure a basic cucumber scenario is running)
1. Document the configuration steps
1. Share the steps with the rest of the team
...
- [ ] Best Practices
    ...
    - [ ] split big problem into several smallers
- [ ] Gather feedback
...
The problem is that while walking in the room, it would be very easy not to have the remote guys not engaged in your discussions.

To avoid that:

1. Try not to have a pair fully in remote
1. Have a solution to easily connect to pair working in Remote (ex: have your laptop and join in the pair's chat from the start)
1. Do not forget the remote pairs
1. Regularly switch between pair sessions (if you have more than one remote team)
...
We use same survey for all teams so that we have all answers grouped together. We always use [this survey](https://forms.office.com/Pages/DesignPageV2.aspx?subpage=design&FormId=cJeDmvzpN0eQXDcPZbDiJAE08ANyJ_BOnREnQ3RgIG9UNE1URzRMNjJSTFJQVDcyMVNXODg5WUU5OSQlQCN0PWcu&Token=ba42f16e00ec46b39cb6ec44688c9a88).
