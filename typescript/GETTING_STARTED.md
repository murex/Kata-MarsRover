# Getting Started with Mars Rover kata in Typescript
...
`File` > `Open` > `Kata-MarsRover` > `typescript`
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-MarsRover` / `typescript`
